			...
			if(xhr.readyState==4&&xhr.status==200){
				...
				var list ='';			
					for(i=0;i<arr.length;i++){
						var obj=arr[i];
						list += "<tr height=50><td width=25%>"+obj.gamename+"</td><td width=25%>"+obj.money+"</td><td width=25%>"+obj.date+"</td><td width=25%>"+obj.state+"</td></tr>";
					}					
				$('table').empty();
...
$(function(){
	if('0个'==$('#rechargenumber').text()){
		var empty = "<div style=\"text-align:center;font-size:12px;color:#ccc;position:relative;left:283px;top:10px;\">没有充值记录</div>";
	...
	$('#navlist li:nth-child(3) a').css('color','#fff').css('background','#09f');

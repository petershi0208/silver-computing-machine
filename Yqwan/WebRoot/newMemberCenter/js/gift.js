...
function getpagetrade(){
		...
		var xhr = getXhr();
		//发送请求
		var uri = '/Yqwan/pagegift.do?num='+page;
			...
			if(xhr.readyState==4&&xhr.status==200){
				...
				var list ='';			
					for(i=0;i<arr.length;i++){
						var obj=arr[i];
						list += "<tr height=50><td width=20%>"+obj.giftname+"</td><td width=20%>"+obj.gamename+"</td><td width=20%>"+obj.cdkey+"</td><td width=20%>"+obj.activationdate+"</td><td width=20%>"+obj.timelimit+"</td></tr>";
					}					
				$('table').empty();
				var firdttr = "<tr height=50><td width=20%>礼包名</td><td width=20%>游戏名字</td><td width=20%>礼包码</td><td width=20%>起始日期</td><td width=20%>截止日期</td></tr>"
...
//判断有无充值记录
$(function(){
	if('0个'==$('#rechargenumber').text()){
		var empty = "<div style=\"text-align:center;font-size:12px;color:#ccc;position:relative;left:283px;top:10px;\">没有礼包记录</div>";
		$('table tr').after(empty);
		$('.center_right_detail_list_page').empty();
	}
	

	//左侧按钮
	$('#navlist li:nth-child(5) img').attr('src','/Yqwan/newMemberCenter/images/jilub.png')
	$('#navlist li:nth-child(5) a').css('color','#fff').css('background','#09f');

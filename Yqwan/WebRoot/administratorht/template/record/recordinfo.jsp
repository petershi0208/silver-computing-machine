											...
											<table id="sample-table-2" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													...
													<tr>
														...
														<td class="hidden-480">${r.describe }</td>
														<td>
															<c:set value="${fn:split(r.gameid, ',') }" var="gameid" />
															<c:forEach items="${gameid }" var="gid">
																<c:forEach items="${gamelist }" var="g">
																	<c:if test="${g.gameid == gid}">
																		${g.gamename }
																	</c:if>
																</c:forEach>
															</c:forEach>
														...
														<td>
															<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
																...
																<a class="green" href="<%=request.getContextPath()%>/administrator/recordsig.do?id=${r.id }">
																	<i class="icon-pencil bigger-130"></i>
																</a>
																<a class="green" href="<%=request.getContextPath()%>/administrator/recordgame.do?id=${r.id }">
																	<i class="bigger-130">查看专辑游戏</i>
																</a>
																<a class="red delete" onclick="javascript:if(confirm('确定要删除此专辑吗？')){alert('删除成功！');window.location.href='/Yqwan/administrator/deletegame.do?id=${g.id }&url=gameinfo';return true;}return false;">

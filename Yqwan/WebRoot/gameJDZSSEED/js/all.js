...
var gamename = "机动战士SEED";
var originalpage = "gameJDZSSEED";   			//对应的Yqwan的webroot根目录下的需要进入的游戏目录名称，若为手机则进入gameBSYPT/phoneLogin
...
$(function(){
	$('.loginMag > div > a:eq(1),#head_recharge a').click(function(){
		...
		if(userid==''){
			...
			$('.loginMag > div > a:eq(1),.lori > a:eq(0),#head_recharge a').attr("href",apiUrl+"/recharge.do?gameid=60000039&serverid=1");
		}
	})	
})
...
function login_login(user,pwd){
	pwd = $.md5(pwd);
	var url = apiUrl + "/logingame.do?g=seed&a=doLogin&userName="+user+"&password="+pwd;
    $.getJSON(url + "&jsoncallback=?", function (json) {
        if (json) {
            if (json.code == 0) {
            	$('#userid').val(user);
	...
	xhr.onreadystatechange=function(){
		...
		if(xhr.readyState==4&&xhr.status==200){
			...
			for(i=0;i<arr.length;i++){
				...
				html += "<div class='newscon'>"+
	        				"<a href=\"http://jdzsseed.199y.com/newsinfo.do?id="+obj.newsid+"\" target=\"_blank\" style=\"cursor: pointer\">  【"+obj.type+"】"+obj.title.substring(0,12)+"</a>"+

...
import com.alipay.util.UtilDate;
	...
	public class ImageDeal {
	     ...
	     * @return
	     */
	    public static boolean modifyImage(Map<String,String> map,List<String> list){
	    	 String data = map.get("avatar_data");
	 		JSONObject jsondata = JSONObject.fromObject(data);
	 		int rotate = (Integer) jsondata.get("rotate");
	 		int width =Integer.parseInt(jsondata.get("width").toString().substring(0,jsondata.get("width").toString().indexOf(".")));
	 		int height =Integer.parseInt(jsondata.get("height").toString().substring(0,jsondata.get("height").toString().indexOf(".")));
	 		int x =Integer.parseInt(jsondata.get("x").toString().substring(0,jsondata.get("x").toString().indexOf(".")));
	 		int y = Integer.parseInt(jsondata.get("y").toString().substring(0,jsondata.get("y").toString().indexOf(".")));		
	 		try {
	 			ImageDeal id = new ImageDeal("d://AllPicture//yqwan//touxiang//"+UtilDate.getMonth()+"//"+list.get(0),"d://AllPicture//yqwan//touxiang//"+UtilDate.getMonth()+"//",list.get(0).substring(0, list.get(0).indexOf(".")),list.get(0).substring(1+list.get(0).indexOf(".")));
	 			if(x<0) x=0;
	 			if(y<0) y=0;
	 			id.spin(rotate);
	 			id.cutting(x, y, width, height);
	 			return true;
	 		} catch (Exception e) {
	 			e.printStackTrace();
	 			return false;

	...
	if (userText.title == "username") {
		...
		if (!(val.match(/^\S+$/))) {
			...
			return false;
				...
				userText.innerHTML = "× 用户名长度2-16个字符之间";
			} else {
				//获取ajax对象
				var xhr = getXhr();
				//发送请求
				var uri = '/Daqian/checkname.do?name='+val;
...
//				});		
		...
		if (!(val.match(/^\S+$/))) {
			...
			var xhr = getXhr();
			//发送请求
			var uri = '/Daqian/checkphone.do?name='+val;
			xhr.open('get',uri,true);
			xhr.onreadystatechange=function(){
				//4,处理服务器返回的数据
				if(xhr.readyState==4&&xhr.status==200){
...
function manageMoneyBtn() {
	$("#manageMoney").css("background-image",
			"url(../registered_files/register_btn_red.png)");
	$("#manageMoneyText").css("color", "white");
	...
	$("#borrowMoney").css("background-image",
			"url(../registered_files/register_btn_gray.png)");
...
function borrowMoneyBtn() {
	$("#borrowMoney").css("background-image",
			"url(../registered_files/register_btn_red.png)");
	$("#borrowMoneyText").css("color", "white");
...
function btnValidate(){
	...
	if($("#uname").val() == "输入用户名"){ 	 //判断密码
		...
		var xhr = getXhr();
		//发送请求
		var uri = '/Daqian/checkname.do?name='+$("#uname").val();
		xhr.open('get',uri,true);
		xhr.onreadystatechange=function(){
			//4,处理服务器返回的数据
			if(xhr.readyState==4&&xhr.status==200){
		...
		/*手机验证*/ 
				  ...
				  //获取ajax对象
					var xhr = getXhr();
					//发送请求
					var uri = '/Daqian/checkphone.do?name='+$("#mobile").val();
					xhr.open('get',uri,true);
					xhr.onreadystatechange=function(){
						//4,处理服务器返回的数据
						if(xhr.readyState==4&&xhr.status==200){
...
//								var manageMoneyUrl = $("#manageMoney").css("background-image");
								if($("#manageMoneyName").val()=="" && $("#borrowMoneyName").val()==""){
								...
								if($("#mobiles").html()=="× 此手机号已被使用"||$("#username").html()=="× 用户名已被注册"){
									return;
								}
...
//		$.ajax({
//			url : "/Daqian/checkname.do?name='" + $("#uname").val() + "'",
//			type : 'post',
//			dataType : 'json',
//			success:function(msg){	
//				if(msg.statusCode == 300){

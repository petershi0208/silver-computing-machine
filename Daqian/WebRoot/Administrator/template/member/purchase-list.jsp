...
<div class="pd-20">
	...
	<div class="text-c"> 
		<form action="purchase-list.do" method="post">
		购买日期范围：
		...
		产品开始日期范围：
		<input type="text" name="date3" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}'})" id="datemin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" name="date4" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d'})" id="datemax" class="input-text Wdate" style="width:120px;">
		<button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜索</button>
		</form>
		<form action="purchase-last.do" method="post">
		购买日期范围：
		<input type="text" name="date5" onFocus="WdatePicker({maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}'})" id="datemin" class="input-text Wdate" style="width:120px;">
		-
		<input type="text" name="date6" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'datemin\')}',maxDate:'%y-%M-%d'})" id="datemax" class="input-text Wdate" style="width:120px;">
		<button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜索</button>		
		</form>
	</div>	
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="70">购买状态</th>
				<th width="100">预计收益</th>
				<th width="100">购买时间</th>
				<th width="130">产品开始时间</th>
				<th width="130">上一次返还利息时间</th>
				<th width="100">年回报率</th>
				<th width="70">付款方式</th>
				<th width="50">购买数量</th>
			</tr>
		</thead>
		<tbody>
			...
			<tr class="text-c">
				<td>${pc.id }</td>
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','360','400')">${pc.username }</u></td>
				...
				<td>${pc.date }</td>
				<td>${pc.startdate }</td>
				<td>${pc.lastbackdate }</td>
				<td>${pc.expectedannual }</td>
				<td>
					<c:if test="${pc.monthoryear==1 }">
						月付
					</c:if>
					<c:if test="${pc.monthoryear==2 }">
						年付
					</c:if>
				</td>
				<td>${pc.amount }</td>

...
<div class="pd-20">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			<c:forEach items="${creditor }" var="cr">
			<tr class="text-c">
				<td class="id">${cr.id }</td>
				<td >${cr.uname }</td>
				<td class="ppname">${cr.idcard }</td>
				<td class="aamount">${cr.address }</td>
				<td class="vvyear">${cr.phone }</td>
				<td>${cr.date }</td>
				<td>
					<a href="/Daqian/ht/creditor-single.do?id=${cr.id }">具体信息</a>
					<a href="/Daqian/ht/creditor-single.do?id=${cr.id }">修改</a>
					<a href="/Daqian/ht/creditor-del.do?id=${cr.id }">删除</a>
				</td>
			</tr>
			</c:forEach>
		</tbody>

...
<div class="pd-20">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a href="javascript:;" onclick="member_add('添加产品利率','subproduct-add.do','','510')" class="btn btn-primary radius">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="100">产品名</th>
				<th width="90">最小金额</th>
				<th width="120">最大金额</th>
				<th width="150">年回报率</th>
				<th width="">到期回报率</th>
		...
		<tbody>
			...
			<tr class="text-c">
				<td>${p.sid }</td>
				<td><u style="cursor:pointer" class="text-primary" onclick="member_show('修改产品利率','subproduct-update.do?id=${p.sid }','10001','600','500')">${p.productname }</u></td>
				<td>${p.minamount }</td>
				<td>${p.maxamount }</td>
				<td>${p.yearreturn }</td>
				<td>${p.duereturn }</td>
				<td>
					<c:if test="${p.monthoryear==1 }">月付</c:if>
					<c:if test="${p.monthoryear==2 }">年付</c:if>
				</td>
				<td>${p.sperson }</td>
				<td>${p.sdate }</td>
				<td class="td-manage"><a title="编辑" href="javascript:;" onclick="member_show('修改产品利率','subproduct-update.do?id=${p.sid }','10001','600','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> </td>
			</tr>
			</c:forEach>
		</tbody>
	</table>
